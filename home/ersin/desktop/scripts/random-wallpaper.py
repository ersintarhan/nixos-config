...
def download_wallpaper(url):
    ...
    try:
        with request.urlopen(req) as response, open(wallpaper_path, "wb") as out_file:
            if response.status != 200:
                print(
                    f"Error: Failed to download image, status code {response.status}",
                    file=sys.stderr,
                )
                return None
...
def cleanup_old_wallpapers():
    ...
    try:
        files = sorted(
            glob(os.path.join(WALLPAPER_DIR, "wallpaper-*.jpg")),
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch and set a random wallpaper from Wallhaven."
    )
    parser.add_argument(
        "key_file",
